...
class MarketplaceContractTests(unittest.TestCase):
    ...
    def test_worktree_lifecycle_and_dispatch_order_are_explicit(self) -> None:
        ...
        setup = self.workflow.index("v1.4.1/scripts/setup.sh", worktree_add, phase_two)
    ...
    def test_gc_baseline_and_breadcrumb_contracts_are_explicit(self) -> None:
        required = (
            "trellis-personal-marketplace/v1.4.1/scripts/setup.sh",
    ...
    def test_release_assets_and_license_are_present(self) -> None:
        ...
        self.assertIn('RELEASE_REF="v1.4.1"', SETUP.read_text(encoding="utf-8"))
    ...
    def test_distributed_scripts_are_listed_in_setup_and_checklists(self) -> None:
        ...
        match = re.search(r'^readonly RELEASE_REF="([^"]+)"$', setup, re.MULTILINE)
        self.assertIsNotNone(match)
        self.assertEqual(match.group(1), "v1.4.1")
